...
contract HelperContract {
     ...
     address CPD_MANAGER = 0x033b7629CeC52a41712C362868f6cd70aEFc0545;
...
contract myCallerContract is HelperContract{
     ...
     address Wrapped_ETH = 0xc778417E063141139Fce010982780140Aa0cD5Ab;   
     ...
     DaiTokenLike dtl = DaiTokenLike(DAI_TOKEN);
     address Wrapped_ETH_Uni =  url.WETH();
     ...
     function getTokenReserves_uni() public view returns (uint, uint) {
        ...
        address pair = uv2.getPair(DAI_TOKEN,Wrapped_ETH_Uni);
     ...
     function myContractsDaiBalance() public view returns (uint){
         return dtl.balanceOf(address(this));
     }
     ...
     function openLockETHAndDraw(uint value) public payable {         
         ...
         (bool success, ) = proxy.call{
                value:msg.value
                 ...
                 DssProxyActions, 
                 abi.encodeWithSignature("openLockETHAndDraw(address,address,address,address,bytes32,uint256)", CPD_MANAGER, MCD_JUG, ETH_JOIN, DAI_JOIN, ilk, value)));
    ...
    function  swapThatShit(uint value, uint exchangeRate) public  {
        address[] memory path = new address[](2);
        path[0] = address(DAI_TOKEN);
        path[1] = url.WETH();
        uint amount_in = value;
        uint amount_out = value/exchangeRate;
        (bool success, ) = UNI_Router.call(abi.encodeWithSignature("swapExactTokensForETH(uint256,uint256,address[],address,uint256)", amount_in, amount_out, path, address(this), block.timestamp + 20));
    }
